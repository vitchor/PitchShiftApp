...
void SmbPitchShifter::stopPitchShifting() {
    shouldStopPitchShifting = true;
}

int SmbPitchShifter::smbPitchShift(float pitchShift, long numSampsToProcess, long fftFrameSize, long osamp, float sampleRate, float *indata, float *outdata)
/*
 ...
 */
{
    ...
    //printf("STARTING SHIFTING PROCESS\n");
	...
	for (i = 0; i < numSampsToProcess; i++){
        ...
        if (!shouldStopPitchShifting) {
            ...
            printf("*** Warning: Pitchshifting process interrupted by TERMINATE signal ***");
            return -1;
        }
    }
    //progress = 0.0;
...
void SmbPitchShifter::smbFft(float *fftBuffer, long fftFrameSize, long sign)
/*
...
double SmbPitchShifter::smbAtan2(double x, double y)
{
    double signx;
    if (x > 0.) signx = 1.;
    else signx = -1.;
